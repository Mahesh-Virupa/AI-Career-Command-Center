    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-fadeIn">
      <div className="bg-white border border-slate-200 rounded-2xl max-w-2xl w-full p-6 text-slate-800 shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-start justify-between border-b border-slate-200 pb-4">
          <div>
            <div className="flex items-center space-x-2">
              <span className="px-2.5 py-0.5 text-[10px] font-mono font-semibold bg-indigo-50 text-indigo-700 border border-indigo-200 rounded uppercase tracking-wider">
                ATS Engine v2.5
              </span>
        ...
        <div className="overflow-y-auto py-4 space-y-5 pr-1 text-xs">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between bg-slate-50 border border-slate-200 rounded-xl p-4 gap-3">
            <div>
              <span className="text-[10px] text-slate-500 uppercase tracking-wider font-mono font-semibold">Matched Résumé Variant</span>
              <div className="flex items-center space-x-2 mt-1">
                <FileText className="h-4 w-4 text-indigo-600" />
                <span className="font-bold text-slate-900 text-sm">{job.matchedResumeName}</span>
              </div>
              <button
                onClick={() => onDownloadResume(job.matchedResumeName)}
                className="text-[11px] text-indigo-600 hover:text-indigo-800 hover:underline flex items-center space-x-1 mt-1 font-mono font-medium"
              >
            ...
            <div className="flex items-center space-x-4 self-end sm:self-center">
              <div className="text-right">
                <span className="text-[10px] text-slate-500 font-mono uppercase font-semibold">Overall Match</span>
                <div className={`text-3xl font-extrabold font-mono ${
                  (tailorSuccess ? 96 : job.atsScore) >= 80 ? 'text-emerald-600' : 'text-rose-600'
                }`}>
            ...
            <div className="bg-emerald-50/60 border border-emerald-200 rounded-xl p-3.5 space-y-2">
              ...
              <div className="flex flex-wrap gap-1.5">
                {matchedKeywords.map((kw, i) => (
                  <span key={i} className="px-2 py-0.5 bg-white border border-emerald-300 text-emerald-800 rounded text-[10px] font-mono font-semibold">
                    ✓ {kw}
                  </span>
                ))}
              </div>
              ...
              {missingKeywords.length > 0 ? (
                <div className="flex flex-wrap gap-1.5">
                  {missingKeywords.map((kw, i) => (
                    <span key={i} className="px-2 py-0.5 bg-white border border-amber-300 text-amber-800 rounded text-[10px] font-mono font-semibold">
                      ✕ {kw}
                    </span>
                  ))}
                </div>
              ) : (
                <p className="text-[11px] text-slate-500 italic">No missing keywords detected. Résumé is fully optimized (95%+ match).</p>
              )}
            </div>
          </div>

          {/* Mandatory Skill Penalty Alert */}
          {isPenalized && (
            <div className="bg-rose-50 border border-rose-200 rounded-xl p-3.5 text-rose-800 space-y-1.5">
          ...
          {ats && (
            <div className="space-y-3 bg-slate-50 border border-slate-200 rounded-xl p-4">
              <h3 className="font-bold text-slate-700 text-[10px] font-mono uppercase tracking-wider">Component Alignment Scores</h3>
              <div className="space-y-2.5">
                ...
                <div>
                  <div className="flex justify-between text-[11px] text-slate-700 font-medium mb-1">
                    ...
                    <span className="font-mono font-bold text-slate-900">{ats.roleScore}/20</span>
                  </div>
                  <div className="w-full bg-slate-200 h-2 rounded-full overflow-hidden">
                    <div className="bg-indigo-600 h-full rounded-full" style={{ width: `${(ats.roleScore / 20) * 100}%` }}></div>
                  </div>
                </div>
                ...
                <div>
                  <div className="flex justify-between text-[11px] text-slate-700 font-medium mb-1">
                    ...
                    <span className="font-mono font-bold text-slate-900">{ats.leadershipScore}/20</span>
                  </div>
                  <div className="w-full bg-slate-200 h-2 rounded-full overflow-hidden">
                    <div className="bg-indigo-600 h-full rounded-full" style={{ width: `${(ats.leadershipScore / 20) * 100}%` }}></div>
                  </div>
                </div>
                ...
                <div>
                  <div className="flex justify-between text-[11px] text-slate-700 font-medium mb-1">
                    ...
                    <span className="font-mono font-bold text-slate-900">{ats.architectureScore}/20</span>
                  </div>
                  <div className="w-full bg-slate-200 h-2 rounded-full overflow-hidden">
                    <div className="bg-indigo-600 h-full rounded-full" style={{ width: `${(ats.architectureScore / 20) * 100}%` }}></div>
                  </div>
                </div>
                ...
                <div>
                  <div className="flex justify-between text-[11px] text-slate-700 font-medium mb-1">
                    ...
                    <span className="font-mono font-bold text-slate-900">{ats.domainScore}/20</span>
                  </div>
                  <div className="w-full bg-slate-200 h-2 rounded-full overflow-hidden">
                    <div className="bg-indigo-600 h-full rounded-full" style={{ width: `${(ats.domainScore / 20) * 100}%` }}></div>
                  </div>
                </div>
                ...
                <div>
                  <div className="flex justify-between text-[11px] text-slate-700 font-medium mb-1">
                    ...
                    <span className="font-mono font-bold text-slate-900">{ats.cloudDevopsScore}/20</span>
                  </div>
                  <div className="w-full bg-slate-200 h-2 rounded-full overflow-hidden">
                    <div className="bg-indigo-600 h-full rounded-full" style={{ width: `${(ats.cloudDevopsScore / 20) * 100}%` }}></div>
                  </div>
                </div>
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-3.5 space-y-1">
            <span className="font-bold text-slate-900">Evaluation Narrative:</span>
            <p className="text-slate-700 leading-relaxed">{ats?.explanation}</p>
          </div>

          {/* Disclaimer */}
          <div className="flex items-start space-x-2 text-[11px] text-slate-500 border-t border-slate-200 pt-3">
